...
class DepotService extends BaseService{
	...
	public function getGoodsLogList($where='',$limit=''){
		...
		$sql.=' g.num_unit,';
		$sql.=' gl.*'; 
		$sql.=' FROM `goods_log` gl';
		$sql.=' LEFT JOIN goods g ON g.goods_id = gl.goods_id';
	...
	public function getGoodsTransactionLogList($start_time,$end_time,$limit=''){
		...
		//用来分组统计日志
		$goodLogSql='';
		$goodLogSql.='SELECT SUM(num) changeNum,goods_id,add_time FROM `goods_log`';
	...
	public function getGoodsTransactionLogCount($start_time,$end_time){
		...
		// 查询sql
		$sql='SELECT COUNT(gl.goods_id) total FROM (SELECT SUM(num) changeNum,goods_id,add_time FROM `goods_log`';
	...
	public function getGoodsCateTransactionLogList($start_time,$end_time,$limit=''){
		...
		$sql.=' (SELECT SUM(num) changeNum1,goods_id,add_time FROM `goods_log` WHERE `type`=3 AND (add_time BETWEEN "'.$start_time.'" AND "'.$end_time.'") GROUP BY goods_id) gl';
		$sql.=' LEFT JOIN `goods` g ON g.`goods_id`=gl.goods_id';
		$sql.=' GROUP BY g.`name`';
	...
	public function getGoodsCateTransactionLogCount($start_time,$end_time){
		...
		$sql.=' (SELECT goods_id FROM `goods_log` WHERE `type`=3 AND (add_time BETWEEN "'.$start_time.'" AND "'.$end_time.'") GROUP BY goods_id) gl';
		$sql.=' LEFT JOIN `goods` g ON g.`goods_id`=gl.goods_id';
	...
	public function getGoodsDeliverGoodsLogList($start_time,$end_time,$limit=''){
		...
		//用来分组统计日志
		$goodLogSql='';
		$goodLogSql.='SELECT SUM(num) changeNum,goods_id,add_time FROM `deliver_goods_log`';
	...
	public function getGoodsDeliverGoodsLogCount($start_time,$end_time){
		...
		// 查询sql
		$sql='SELECT COUNT(gl.goods_id) total FROM (SELECT SUM(num) changeNum,goods_id,add_time FROM `deliver_goods_log`';
	...
	public function getGoodsCateDeliverGoodsLogList($start_time,$end_time,$limit=''){
		...
		$sql.='SELECT g.`name`,SUM(gl.changeNum1) changeNum,g.`add_time` FROM';
		$sql.=' (SELECT SUM(num) changeNum1,goods_id,add_time FROM `deliver_goods_log` WHERE (add_time BETWEEN "'.$start_time.'" AND "'.$end_time.'") GROUP BY goods_id) gl';
		$sql.=' LEFT JOIN `goods` g ON g.`goods_id`=gl.goods_id';
		$sql.=' GROUP BY g.`name`';
	...
	public function getGoodsCateDeliverGoodsLogCount($start_time,$end_time){
		...
		$sql='';
		$sql.='SELECT COUNT(tgc.`name`) total FROM ( SELECT g.`name` FROM';
		$sql.=' (SELECT goods_id FROM `deliver_goods_log` WHERE (add_time BETWEEN "'.$start_time.'" AND "'.$end_time.'") GROUP BY goods_id) gl';
		$sql.=' LEFT JOIN `goods` g ON g.`goods_id`=gl.goods_id';
		$sql.=' GROUP BY g.`name`) tgc';
